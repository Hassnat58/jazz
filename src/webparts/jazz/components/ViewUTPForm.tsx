...
const ViewUTPForm: React.FC<{
  ...
  const generatePDF = async () => {
    ...
    // Capture canvas with reduced scale
    const canvas = await html2canvas(element, {
      scale: 0.7, // smaller → faster, smaller file
      useCORS: true,
  ...
  return (
    <div id="pdf-container">
        ...
        <table className={styles.detailTable}>
          <tbody>
            ...
            <td>{data.ERMUniqueNumbering}</td> */}
              ...
              <td>
                {data.UTPDate
                  ? new Date(data.UTPDate).toLocaleDateString()
                  : ""}
              </td>
            </tr>
      ...
      <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
        ...
        {/* <button
          onClick={() => {
            const subject = `UTP Details: ${data?.UTPId}`;
            const body = `Please find the UTP details: ${window.location.href}`;
